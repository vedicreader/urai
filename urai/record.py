...
# %% auto #0
__all__ = ['CHAT_CACHE', 'KEY_VERSION', 'is_transient', 'RecordCache', 'as_resp', 'canon_msg', 'CachedChat']
...
def as_resp(r):
    ...
    on a string raises rather than saying so, which used to poison the cache with a `ValueError`
    that then replayed forever.
    """
    if isinstance(r, str): return {'role': 'assistant', 'content': r}
    try: return dict(r)
    except (TypeError, ValueError): return {'role': 'assistant', 'content': str(r)}
